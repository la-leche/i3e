...
bool parse_tick_line(std::string_view line, Tick& tick) {
    ...
    if (third == std::string_view::npos) {
        ...
        return parse_number(line.substr(0, first), tick.price)
            && parse_number(line.substr(first + 1, second - first - 1), tick.volume)
    ...
    return parse_number(line.substr(first + 1, second - first - 1), tick.price)
        && parse_number(line.substr(second + 1, third - second - 1), tick.volume)
...
void calculate_range_bars(std::string_view file_view,
    ...
    if (has_current && current.volume > 0) bars.push_back(current);
}
