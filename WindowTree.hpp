...
enum class Session { RTH, ETH };

class ChartWindow {
public:
    virtual ~ChartWindow() = default;
    [[nodiscard]] virtual std::string_view title() const noexcept = 0;
    virtual void render(WINDOW* window, std::string_view pane_id,
                        const std::vector<RangeBar>& bars,
                        std::size_t offset, bool active) const = 0;
...
public:
    ...
    void render(WINDOW*, std::string_view, const std::vector<RangeBar>&,
...
public:
    explicit TpoWindow(Session session = Session::ETH) : session_(session) {}
    [[nodiscard]] std::string_view title() const noexcept override;
    void render(WINDOW*, std::string_view, const std::vector<RangeBar>&,
                std::size_t, bool) const override;
...
void render_tree(WindowNode& node, const std::vector<RangeBar>& bars,
                 std::size_t offset, const WindowNode* active_leaf);
